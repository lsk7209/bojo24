...
## 현재 작업
K-Startup/MSS 창업지원 데이터 연동 구현 완료, 공공데이터 게이트웨이 403 확인
## 최근 변경 (최근 5개만)
...
- 05-06: `/startup`, `/api/startup`, `startup_items` Turso 스키마, `fetch:startup` 추가
...
- 창업지원 API: 승인 직후에도 403이면 공공데이터포털 키 반영 대기 또는 운영계정/활용신청 범위 확인
## 주의
